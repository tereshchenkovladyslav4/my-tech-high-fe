import { S as SYMBOL_TO_STRING_TAG, n as nodejsCustomInspectSymbol, i as isSource, a as Source, b as inspect, c as invariant$1, d as checkDocument, e as isField, _ as __assign, f as __spreadArray, g as createFragmentMap, h as getFragmentDefinitions, j as getOperationDefinition, k as getFragmentDefinition, l as isInlineFragment, m as getMainDefinition, o as isNonNullObject, O as Observable, p as __extends, A as ApolloLink, I as InvariantError, q as maybe, r as __rest, s as getFragmentQueryDocument, t as isReference, u as resultKeyNameFromField, v as makeReference, w as getDefaultValues, x as getQueryDefinition, y as getFragmentFromSelection, z as getTypenameFromResult, B as getStoreKeyName, C as argumentsObjectFromField, D as storeKeyNameFromField, E as __awaiter, F as __generator, G as getOperationName, H as isDocumentNode } from '../common/ApolloLink-153f28d0.js';
    ...
    if (directives && directives.length) {
        directives.forEach(function (directive) {
            ...
            __DEV__ ? invariant$1(ifValue &&
                (ifValue.kind === 'Variable' || ifValue.kind === 'BooleanValue'), "Argument for the @" + directiveName + " directive must be a variable or a boolean value.") : invariant$1(ifValue &&
...
function assertSelectionSetForIdValue(store, field, fieldValue) {
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (isNonNullObject(value)) {
                __DEV__ ? invariant$1(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value) : invariant$1(!isReference(value), 7);
        ...
        selections.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (value !== void 0) {
                    ...
                    __DEV__ && invariant$1.error(("Missing field '" + resultKeyNameFromField(selection) + "' while writing result " + JSON.stringify(result, null, 2)).substring(0, 1000));
                }
    ...
    ObservableQuery.prototype.refetch = function (variables) {
        ...
        if (__DEV__ && variables && hasOwnProperty$1.call(variables, "variables")) {
            ...
            if (!vars || !vars.some(function (v) { return v.variable.name.value === "variables"; })) {
                __DEV__ && invariant$1.warn("Called refetch(" + JSON.stringify(variables) + ") for query " + (((_a = queryDef.name) === null || _a === void 0 ? void 0 : _a.value) || JSON.stringify(queryDef)) + ", which does not declare a $variables variable.\nDid you mean to call refetch(variables) instead of refetch({ variables })?");
            }
        }
...
function logMissingFieldErrors(missing) {
    if (__DEV__ && isNonEmptyArray(missing)) {
        __DEV__ && invariant$1.debug("Missing cache result fields: " + missing.map(function (m) { return m.path.join('.'); }).join(', '), missing);
    }
}
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.query = function (options, queryId) {
        ...
        __DEV__ ? invariant$1(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.') : invariant$1(options.query, 16);
...
var ApolloClient = (function () {
    ...
    ApolloClient.prototype.query = function (options) {
        ...
        __DEV__ ? invariant$1(options.fetchPolicy !== 'cache-and-network', 'The cache-and-network fetchPolicy does not work with client.query, because ' +
            ...
            'using a different fetchPolicy, such as cache-first or network-only.') : invariant$1(options.fetchPolicy !== 'cache-and-network', 10);
...
var ApolloProvider = function (_a) {
    ...
    return react.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        __DEV__ ? invariant$1(context.client, 'ApolloProvider was not passed a client instance. Make ' +
            'sure you pass in your client via the "client" prop.') : invariant$1(context.client, 28);
...
function parser(document) {
    ...
    __DEV__ ? invariant$1(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
        "DocumentNode. You may need to use 'graphql-tag' or another method " +
        "to convert your operation into a document") : invariant$1(!!document && !!document.kind, 34);
    ...
    __DEV__ ? invariant$1(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
        ...
        "You can use 'compose' to join multiple operation types to a component") : invariant$1(queries.length + mutations.length + subscriptions.length <= 1, 36);
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    __DEV__ ? invariant$1(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
        (definitions.length + " definitions. ") +
        "You can use 'compose' to join multiple operation types to a component") : invariant$1(definitions.length === 1, 37);
...
var OperationData = (function () {
    ...
    OperationData.prototype.refreshClient = function () {
        ...
        __DEV__ ? invariant$1(!!client, 'Could not find "client" in the context or passed in as an option. ' +
            ...
            'ApolloClient instance in via options.') : invariant$1(!!client, 29);
        var isNew = false;
    ...
    OperationData.prototype.verifyDocumentType = function (document, type) {
        ...
        __DEV__ ? invariant$1(operation.type === type, "Running a " + requiredOperationName + " requires a graphql " +
            (requiredOperationName + ", but a " + usedOperationName + " was used instead.")) : invariant$1(operation.type === type, 30);
    };
...
export { ApolloClient, ApolloProvider, HttpLink, InMemoryCache, concat, gql, useLazyQuery, useMutation, useQuery };
