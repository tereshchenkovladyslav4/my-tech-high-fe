...
import { createOrUpdateTitleMutation, getSubjectsQuery } from '@mth/screens/Admin/Curriculum/CourseCatalog/services'
...
import { ScheduleBuilder, StateCourseCord, Subject, Title, TitleEditProps } from '../types'

const TitleEdit: React.FC<TitleEditProps> = ({ schoolYearId, schoolYearData, item, refetch, setShowEditModal }) => {
  const { me } = useContext(UserContext)

  const [subjectsItems, setSubjectsItems] = useState<CheckBoxListVM[]>([])
  const [isSubmitted, setIsSubmitted] = useState<boolean>(false)
  const [scheduleBuilder, setScheduleBuilder] = useState<ScheduleBuilder | undefined>()
  const [initialValues, setInitialValues] = useState<Title>(defaultTitleFormData)
  ...
  const { data: scheduleBuilderData } = useQuery(getScheduleBuilder, {
    variables: {
  ...
  useEffect(() => {
    if (scheduleBuilderData?.schoolyear_getcurrent?.ScheduleBuilder) {
      setScheduleBuilder(scheduleBuilderData.schoolyear_getcurrent.ScheduleBuilder)
    }
  }, [scheduleBuilderData])
