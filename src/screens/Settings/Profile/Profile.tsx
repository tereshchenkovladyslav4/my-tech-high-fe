...
export const Profile = ({ handleIsFormChange }) => {
	...
	const onSave = async () => {
		submitUpdate({
			...
			.catch((err) => {
				setOpenSaveAlert({ message: 'The email is already in use. Please use different email address.', status: 'error', open: true })
	...
	const formik = useFormik({
		...
		onSubmit: async () => {
			...
			if(me.email != formik.values.email) {
				setWarningModalOpen({
					title: 'Change Email',
					subtitle: 'Changing your email requires a new verification link to be sent. You will be logged out until you verify your new email. Would you like to proceed?',
					callback: onSave
				});
			}
			else
